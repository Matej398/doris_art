...
import type { GalleryData, GalleryImage } from '@/lib/admin/validation';

export default function WallPaintingsPage() {
  ...
  const fetchImages = useCallback(async () => {
    try {
      const response = await fetch('/api/admin/wall-paintings');
      const data: GalleryData = await response.json();
      setImages(data.images);
    } catch (error) {
      console.error('Error fetching wall paintings:', error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function handleAddImage(src: string) {
    ...
    try {
      const response = await fetch('/api/admin/wall-paintings', {
        ...
        body: JSON.stringify({ src, alt: 'Stenska poslikava' }),
      });
  ...
  async function handleDelete(id: number) {
    ...
    setDeleting(id);
    try {
      const response = await fetch(`/api/admin/wall-paintings/${id}`, { method: 'DELETE' });
      if (response.ok) {
  ...
  async function handleUpdateAlt(id: number, alt: string) {
    try {
      await fetch(`/api/admin/wall-paintings/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleReorder(fromIndex: number, toIndex: number) {
    ...
    try {
      await fetch('/api/admin/wall-paintings', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Stenske poslikave</h1>
        <p className="text-gray-500 mt-1">Upravljanje slik za stran stenskih poslikav</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-8 text-center">
          <p className="text-gray-500">Ni slik stenskih poslikav. Naložite svojo prvo sliko zgoraj.</p>
        </div>
      ) : (
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">
              Slike stenskih poslikav ({images.length})
            </h2>
