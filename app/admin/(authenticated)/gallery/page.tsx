...
export default function GalleryPage() {
  ...
  async function handleAddImage(src: string) {
    ...
    try {
      const response = await fetch('/api/admin/gallery', {
        ...
        body: JSON.stringify({ src, alt: 'Gallery image' }),
      });
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Galerija</h1>
        <p className="text-gray-500 mt-1">Upravljanje glavne galerije</p>
      </div>
      ...
      {loading ? (
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-8 text-center">
          <div className="animate-pulse text-gray-500">Nalaganje galerije...</div>
        </div>
      ) : images.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-8 text-center">
          <p className="text-gray-500">Ni slik v galeriji. Naložite svojo prvo sliko zgoraj.</p>
        </div>
      ) : (
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">
              Slike v galeriji ({images.length})
            </h2>
